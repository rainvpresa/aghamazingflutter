...
class AuthApi {
  ...
  // UPDATED: Now handles 204 No Content and empty responses
  Future<String> registerUser({
    ...
    if (resp.isEmpty) {
      // Registration succeeded but no key returned
      // Return a placeholder key or empty string
    ...
    try {
      ...
      if (data is Map && data['key'] is String && (data['key'] as String).isNotEmpty) {
        ...
        // Registration succeeded but no key in response
        return 'registration_success';
      }
    } catch (e) {
      // If we can't parse JSON but registration succeeded (status 2xx), return success
      return 'registration_success';
    }
  }
